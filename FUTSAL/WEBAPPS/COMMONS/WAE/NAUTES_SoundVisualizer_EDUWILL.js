...
if (WAVE_DISPLAY_ONLY) {
	STR_HTML += '<div style="width:100%; height:100%; padding:0px;"">';
		...
		STR_HTML += '<div style="width:' + (100 - WIDTH_RATIO_PLAYLIST) + '%; height:100%; padding:0px; float:left;">';
			STR_HTML += '<canvas style="width:100%;height:100%;background-color:#555;" class="CANVAS_NAUTES_SOUND_VISUALIZER"></canvas>';
		STR_HTML += '</div>';
	STR_HTML += '</div>';
} else {
	STR_HTML += '<div style="width:100%; height:10%; padding:0px; text-align: center; background:#aaa;">';
		STR_HTML += '<table>';
			...
			STR_HTML += '<tr>';
				STR_HTML += '<th id="NAUTES_WAE_AUDIO_POSTION">';
				STR_HTML += '</th>';
				STR_HTML += '<td>';
					STR_HTML += '<div id="NAUTES_WAE_PROGRESSBAR" style="margis:0px;"></div>';
				STR_HTML += '</td>';
			STR_HTML += '</tr>';
		STR_HTML += '</table>';
	STR_HTML += '</div>';
	STR_HTML += '<div style="width:100%; height:75%; padding:0px;"">';
		STR_HTML += '<div style="width:' + WIDTH_RATIO_PLAYLIST + '%; height:100%; padding:0px; float:left; overflow:auto;" id="NAUTES_WAE_PLAYLIST">';
		STR_HTML += '</div>';
		STR_HTML += '<div style="width:' + (100 - WIDTH_RATIO_PLAYLIST) + '%; height:100%; padding:0px; float:left;">';
			STR_HTML += '<div style="width:100%; height:20%; padding:0px;">';
				STR_HTML += '<div id="NAUTES_WAE_PLAY_SPEED"></div>';
			STR_HTML += '</div>';
		...
		STR_HTML += '</div>';
	STR_HTML += '</div>';
	STR_HTML += '<div style="width:100%; height:15%; padding:0px; text-align:left;">';
...
if (!WAVE_DISPLAY_ONLY) {
	$('#NAUTES_WAE_PROGRESSBAR').jqxProgressBar({animationDuration:0, showText:true, value:1, max:100, width: '100%', height: '100%'});	
	$("#NAUTES_WAE_PROGRESSBAR").jqxProgressBar('val', 0);
	$("#NAUTES_WAE_AUDIO_POSTION").html('00:00:00.000');
	$("#NAUTES_WAE_PLAY_SPEED").jqxSlider({
        width: '100%',
        height: 30,
        mode: 'fixed',
...
//			console.log(ratio_x_adjusted);
	...
	this.SMOOTHING = 0.0;
	this.FFT_SIZE = 512;
	this.CANVAS = document.querySelector('.CANVAS_NAUTES_SOUND_VISUALIZER');
...
//	this.TRACK_AUDIO_LENGTH	= -1;
	...
	this.WAE_EE.on('loadprogress', function (percent, src) {
		$("#NAUTES_WAE_PROGRESSBAR").jqxProgressBar('val', percent);
		$("#NAUTES_WAE_AUDIO_POSTION").html(sprintf('%5.2f', percent) + ' %');
	}).on('timeupdate', function (playbackPosition) {
		$("#NAUTES_WAE_AUDIO_POSTION").html(cueFormatters('hh:mm:ss.uuu')(playbackPosition));
		$("#NAUTES_WAE_PROGRESSBAR").jqxProgressBar('val', playbackPosition / _THIS.MAX_AUDIO_LENGTH * 100);
...
//			console.log("_THIS.isLooping!");
	...
	this.displayDownloadLink = function (link) {
		...
		var $link = $("<a/>", {
			'href': link,
			'download': 'MIX_WAV_' + dateString + '.wav',
			'text': 'Download MIX_WAV_' + dateString,
