function sortByKey(array, key) {
	...
	for (var i=0; i<TEMP.length; i++) {
		if (TEMP[i] != null) RESULT.push(TEMP[i]);
	}
	return RESULT;
}

function ADJUST_MULTI_SELECTION(args_MULTI_timeSelection) {
	var SORTED_MULTI_timeSelection = sortByKey(args_MULTI_timeSelection, 'start');
	for(var i=1; i<SORTED_MULTI_timeSelection.length; i++) {
		if (SORTED_MULTI_timeSelection[i-1].end > SORTED_MULTI_timeSelection[i].start) {
			SORTED_MULTI_timeSelection[i-1].end = SORTED_MULTI_timeSelection[i].end;
		}
	}
	var RESULT = [];
	for(var i=0; i<SORTED_MULTI_timeSelection.length; i++) {
		if (SORTED_MULTI_timeSelection[i].start != SORTED_MULTI_timeSelection[i].end) {
			if ((i>0) && (SORTED_MULTI_timeSelection[i-1].end < SORTED_MULTI_timeSelection[i].end)) {
				RESULT.push(SORTED_MULTI_timeSelection[i]);
			}
		}
	}
	return RESULT;
...
function NAUTES_SoundVisualizer(CONFIG) {
	
	var _THIS = this;
	this.DOM_ID_FOR_RENDERING = CONFIG.DOM_ID_FOR_RENDERING;
	this.DOM_FOR_RENDERING = $('#' + CONFIG.DOM_ID_FOR_RENDERING);
	var WAVE_DISPLAY_ONLY = false;
	var WIDTH_RATIO_PLAYLIST = 75;
	var SAVE_AS = '';
	var WIDTH_TRACK_CONTROLLER = 200;
	var TRACK_CONTROLLER_ENABLE = true;
	var MAX_PLAY_SPEED = 900;
	var CTL_BUTTON = true;
	var EDITABLE = true;
	var IS_MOBILE = detectmob();
	if ('SAVE_AS' in CONFIG) SAVE_AS = CONFIG.SAVE_AS;
	if ('TRACK_CONTROLLER_ENABLE' in CONFIG) TRACK_CONTROLLER_ENABLE = CONFIG.TRACK_CONTROLLER_ENABLE;
	if ('MAX_PLAY_SPEED' in CONFIG) MAX_PLAY_SPEED = CONFIG.MAX_PLAY_SPEED;
	if ('EDITABLE' in CONFIG) EDITABLE = CONFIG.EDITABLE;
	if ('WIDTH_RATIO_PLAYLIST' in CONFIG) WIDTH_RATIO_PLAYLIST = CONFIG.WIDTH_RATIO_PLAYLIST;
	if ('WAVE_DISPLAY_ONLY' in CONFIG) WAVE_DISPLAY_ONLY = CONFIG.WAVE_DISPLAY_ONLY;
	if ('MOBILE_LIKE' in CONFIG) IS_MOBILE = CONFIG.MOBILE_LIKE;
	if ('CTL_BUTTON' in CONFIG) CTL_BUTTON = CONFIG.CTL_BUTTON;
	if (WAVE_DISPLAY_ONLY) WIDTH_RATIO_PLAYLIST = 0;

	var TEST_CONTEXT = null;
	try {
		// Fix up for prefixing
		window.AudioContext = window.AudioContext||window.webkitAudioContext;
		TEST_CONTEXT = new AudioContext();
	} catch(e) {
	}	
...
//	if (!navigator.mediaDevices) {
	if (TEST_CONTEXT == null) {
		...
		STR_HTML += '<tr>';
		STR_HTML += '<th>';
		STR_HTML += '<b>Your BROWSER does not support Web Audio feature</b><br/><br/>';
		STR_HTML += 'USE Chrome, Edge ETC.';
		STR_HTML += '</th>';
		STR_HTML += '</tr>';
		STR_HTML += '</table>';
		$(this.DOM_FOR_RENDERING).html(STR_HTML);
		return;
	}

	$(this.DOM_FOR_RENDERING).on('contextmenu', function () {
		return false;
	});

	// shim layer with setTimeout fallback
	window.NAUTES_SOUND_VISUALIZER_UPDATE_CANVAS = (function(){
	...
	STR_HTML  = '';
if (WAVE_DISPLAY_ONLY) {
	STR_HTML += '<div style="width:100%; height:100%; padding:0px;"">';
		STR_HTML += '<div style="width:' + WIDTH_RATIO_PLAYLIST + '%; height:100%; padding:0px; float:left; overflow:auto;" id="NAUTES_WAE_PLAYLIST">';
		STR_HTML += '</div>';
		STR_HTML += '<div style="width:' + (100 - WIDTH_RATIO_PLAYLIST) + '%; height:100%; padding:0px; float:left;">';
			STR_HTML += '<canvas style="width:100%;height:100%;background-color:#555;" class="CANVAS_' + _THIS.DOM_ID_FOR_RENDERING + '"></canvas>';
		STR_HTML += '</div>';
	STR_HTML += '</div>';
} else {
	STR_HTML += '<div style="width:100%; height:10%; padding:0px; text-align: center; background:#aaa;">';
		STR_HTML += '<table>';
	if (IS_MOBILE) {
			STR_HTML += '<col width="30%"/>';
	} else {
			STR_HTML += '<col width="15%"/>';
	}
			STR_HTML += '<tr>';
				STR_HTML += '<th id="WAE_AUDIO_POSTION_' + _THIS.DOM_ID_FOR_RENDERING + '">';
				STR_HTML += '</th>';
				STR_HTML += '<td>';
					STR_HTML += '<div id="WAE_PROGRESSBAR_' + _THIS.DOM_ID_FOR_RENDERING +'" style="margis:0px;"></div>';
				STR_HTML += '</td>';
			STR_HTML += '</tr>';
		STR_HTML += '</table>';
	STR_HTML += '</div>';
	STR_HTML += '<div style="width:100%; height:80%; padding:0px;"">';
	if (IS_MOBILE) {
		STR_HTML += '<div style="width:100%; height:40%; padding:0px; float:left;">';
			STR_HTML += '<div style="width:100%; height:20%; padding:0px;">';
				STR_HTML += '<div id="WAE_PLAY_SPEED_' + _THIS.DOM_ID_FOR_RENDERING + '"></div>';
			STR_HTML += '</div>';
			STR_HTML += '<div style="width:100%; height:80%; padding:0px;">';
				STR_HTML += '<canvas style="width:100%;height:100%;background-color:#555;" class="CANVAS_' + _THIS.DOM_ID_FOR_RENDERING + '"></canvas>';
			STR_HTML += '</div>';
		STR_HTML += '</div>';
		STR_HTML += '<div style="width:100%; height:60%; padding:0px; float:left; overflow:auto;" id="NAUTES_WAE_PLAYLIST">';
		STR_HTML += '</div>';
	} else {
		STR_HTML += '<div style="width:' + WIDTH_RATIO_PLAYLIST + '%; height:100%; padding:0px; float:left; overflow:auto;" id="NAUTES_WAE_PLAYLIST">';
		STR_HTML += '</div>';
	...
	STR_HTML += '</div>';
	STR_HTML += '<div style="width:100%; height:10%; padding:0px; text-align:left;">';
	if (CTL_BUTTON) {
		STR_HTML += '<div class="btn-group">';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE"	class="BTN_NAUTES_WAE btn-pause btn btn-warning"><i class="fa fa-play"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_STOP"		class="BTN_NAUTES_WAE btn-stop btn btn-danger"><i class="fa fa-stop"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_REWIND"		class="BTN_NAUTES_WAE btn-rewind btn btn-success"><i class="fa fa-fast-backward"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_FASTFORWARD"	class="BTN_NAUTES_WAE btn-fast-forward btn btn-success"><i class="fa fa-fast-forward"></i></span>';
		STR_HTML += '</div>';
		STR_HTML += '<div class="btn-group">';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_ZOOM_IN"		title="WAVE ZOOM-IN" class="BTN_NAUTES_WAE btn-zoom-in btn btn-default"><i class="fa fa-search-plus"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_ZOOM_OUT"	title="WAVE ZOOM-OUT" class="BTN_NAUTES_WAE btn-zoom-out btn btn-default"><i class="fa fa-search-minus"></i></span>';
		STR_HTML += '</div>';
	}
	if (!IS_MOBILE) {
		STR_HTML += '<div class="btn-group btn-playlist-state-group">';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_MODE_CURSOR"	class="BTN_NAUTES_WAE btn-cursor btn btn-default active" title="POSITION MODE"><i class="fa fa-headphones"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_MODE_SELECT"	class="BTN_NAUTES_WAE btn-select btn btn-default" title="RANGE MODE"><i class="fa fa-italic"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_MODE_SHIFT"	class="BTN_NAUTES_WAE btn-shift  btn btn-default" title="SHIFT MODE"><i class="fa fa-arrows-h"></i></span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_MODE_DELETE"	class="BTN_NAUTES_WAE btn-trash  btn btn-default" title="TRACK DELETE "><i class="fa fa-trash-o"></i></span>';
		STR_HTML += '</div>';
		STR_HTML += '<div class="btn-group btn-select-state-group">';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_TOGGLE_LOOP"	class="BTN_NAUTES_WAE btn-loop btn btn-default"		title="LOOP_PLAY SELECTIONS">';
				STR_HTML += '<i class="fa fa-repeat"></i>';
			STR_HTML += '</span>';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_CLEAR"	class="BTN_NAUTES_WAE btn btn-clear btn-danger" title="CLEAR" >CLEAR</span>';
		STR_HTML += '</div>';
	}
	if (EDITABLE) {
		if (!IS_MOBILE) {
			STR_HTML += '<div class="btn-group">';
				STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_SLICE"	class="BTN_NAUTES_WAE btn-trim-audio btn btn-primary"	title="SLICE" >SLICE</span>';
			STR_HTML += '</div>';
		}
		STR_HTML += '<div class="btn-group">';
			STR_HTML += '<span id="BTN_NAUTES_WAE_EXEC_DOWNLOAD" title="DOWNLOAD AS MIX-WAV"';
			    STR_HTML += 'class="BTN_NAUTES_WAE btn btn-download btn-primary">';
				STR_HTML += '<i class="fa fa-download"></i>';
			STR_HTML += '</span>';
		STR_HTML += '</div>';
	}
	STR_HTML += '</div>';
...
if (!WAVE_DISPLAY_ONLY) {
	$('#WAE_PROGRESSBAR_' + _THIS.DOM_ID_FOR_RENDERING).jqxProgressBar({animationDuration:0, showText:true, value:1, max:100, width: '100%', height: '100%'});	
	$("#WAE_PROGRESSBAR_" + _THIS.DOM_ID_FOR_RENDERING).jqxProgressBar('val', 0);
	$("#WAE_AUDIO_POSTION_" + _THIS.DOM_ID_FOR_RENDERING).html('00:00:00.000');
	$("#WAE_PLAY_SPEED_" + _THIS.DOM_ID_FOR_RENDERING).jqxSlider({
        width: '100%',
        height: 30,
        mode: 'fixed',
        min: 10, max: MAX_PLAY_SPEED, step: 10, value: 100,
        ticksFrequency: 50,
        showRange: true,
        tooltip: true,
		tooltipFormatFunction: function(value) {
			return sprintf("%3.1f X", (value / 100));
		}
	}).on('change', function (event) {
		var value = event.args.value;
		if (_THIS.WAE_PLAYLIST.isPlaying()) {
			_THIS.WAE_EE.emit("stop");
			_THIS.WAE_EE.emit("play");
		}
		_THIS.WAE_PLAYLIST.playSpeed = value / 100;
	});
}

	this.selectActive = function(node) {
	...
	this.toggleActive = function(node) {
		var active = node.parentNode.querySelectorAll('.active');
		if (active.length > 0) {
	...
	$container.on("click", ".btn-cursor", function() {
	  _THIS.WAE_EE.emit("statechange", "cursor");
	  _THIS.selectActive(this);
	}).on("click", ".btn-select", function() {
	  _THIS.WAE_EE.emit("statechange", "select");
	  _THIS.selectActive(this);
	}).on("click", ".btn-shift", function() {
	  _THIS.WAE_EE.emit("statechange", "shift");
	  _THIS.selectActive(this);
	}).on("click", ".btn-trash", function() {
	  _THIS.WAE_EE.emit("track-delete");
	});
	

if (!WAVE_DISPLAY_ONLY) {
	$(".BTN_NAUTES_WAE").on('click', function () {
		var ID = $(this).attr('ID');
		if (ID == 'DUMMY') {
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE') {
			if (_THIS.WAE_PLAYLIST.isPlaying()) {
				_THIS.WAE_EE.emit("pause");
				$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
			} else {
				_THIS.WAE_EE.emit("automaticscroll", true);
				_THIS.WAE_EE.emit("play");
				$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-pause">');
			}
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_STOP') {
			$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
			_THIS.WAE_EE.emit("stop");
			_THIS.WAE_EE.emit("automaticscroll", false);
			if (_THIS.isLooping) {
				_THIS.toggleActive(document.querySelector('#BTN_NAUTES_WAE_EXEC_TOGGLE_LOOP'));
				_THIS.isLooping = !_THIS.isLooping;
			}
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_REWIND') {
			$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
			_THIS.WAE_EE.emit("stop");
			_THIS.WAE_EE.emit("rewind");
			_THIS.WAE_EE.emit("automaticscroll", false);
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_FASTFORWARD') {
			$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
			_THIS.WAE_EE.emit("stop");
			_THIS.WAE_EE.emit("fastforward");
			_THIS.WAE_EE.emit("automaticscroll", false);
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_ZOOM_IN') {
			_THIS.WAE_EE.emit("zoomin");
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_ZOOM_OUT') {
			_THIS.WAE_EE.emit("zoomout");
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_TOGGLE_LOOP') {
			if (Math.abs(_THIS.SELECT_START - _THIS.SELECT_END) > 0.1) {
				_THIS.toggleActive(this);
				_THIS.isLooping = !_THIS.isLooping;
				if (_THIS.isLooping) {
						$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-pause">');
						_THIS.playoutPromises = _THIS.WAE_PLAYLIST.play(_THIS.SELECT_START, _THIS.SELECT_END);
				}
			} else {
				_THIS.displaySoundStatus(sprintf('RANGE [%5.2f] IS TOO NARROW!', Math.abs(_THIS.SELECT_START - _THIS.SELECT_END)));
				if (_THIS.isLooping) {
					_THIS.toggleActive(this);
					_THIS.isLooping = !_THIS.isLooping;
				}
			}
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_SLICE') {
			_THIS.WAE_EE.emit("trim");
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_CLEAR') {
			_THIS.WAE_EE.emit("automaticscroll", false);
			_THIS.WAE_EE.emit("clear");
			$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
		} else if (ID == 'BTN_NAUTES_WAE_EXEC_DOWNLOAD') {
			_THIS.WAE_EE.emit("startaudiorendering", "wav");
		}
	});
}
	...
	$(this.DOM_FOR_RENDERING).on('dragenter', function (e) {
		e.preventDefault();
		...
		for (var i = 0; i < dropEvent.dataTransfer.files.length; i++) {
			_THIS.WAE_EE.emit("newtrack", dropEvent.dataTransfer.files[i]);
		}
	}).on(mousewheelevent, function (e) {
		var evt=window.event || e;								 //equalize event object
		var delta=evt.detail? evt.detail*(-120) : evt.wheelDelta;//check for detail first so Opera uses that instead of wheelDelta
		var WIDTH_DOM =$(_THIS.DOM_FOR_RENDERING).width();
		var WIDTH_TRACK = WIDTH_DOM * WIDTH_RATIO_PLAYLIST / 100 - WIDTH_TRACK_CONTROLLER;
		var x_adjusted = evt.x - WIDTH_TRACK_CONTROLLER;
		if ((x_adjusted > 0) && (x_adjusted < WIDTH_TRACK)) {
			e.preventDefault();
			var ratio_x_adjusted = x_adjusted * 100 / WIDTH_TRACK;
//			console.log(ratio_x_adjusted);
			if (delta>0) {
				_THIS.WAE_EE.emit("zoomin");
			} else {
				_THIS.WAE_EE.emit("zoomout");
	...
	this.BINARY_STRING_TO_Array = function (args_BINARY_STRING) {
		var RESULT = new Array();

		var _ArrayBuffer	= new ArrayBuffer(args_BINARY_STRING.length);
		var _DataView		= new DataView(_ArrayBuffer);
		var _BufferView		= new Uint8Array(_ArrayBuffer);
		for (var i=0; i<args_BINARY_STRING.length; i++) {
			_BufferView[i]	= args_BINARY_STRING.charCodeAt(i);
		}
		for (var i=0; i<(args_BINARY_STRING.length / 2); i++) {
			RESULT.push(_DataView.getInt16(i*2, true));
		}
		return RESULT;
	}

	this.SOUND_QUEUE_SAMPLE_RATE	= 48000;
	this.SOUND_QUEUE_DURATION		= 60 * 5; // 5 MINUTES OF SOUND DURATION;
	this.SOUND_QUEUE_SIZE			= this.SOUND_QUEUE_SAMPLE_RATE * this.SOUND_QUEUE_DURATION;
	this.SOUND_QUEUE				= new Float32Array(this.SOUND_QUEUE_SIZE);
	this.SOUND_QUEUE_INDEX_HEAD		= 0;
	this.SOUND_QUEUE_INDEX_TAIL		= 0;


	this.CLEAR_SOUND_QUEUE = function () {
		this.SOUND_QUEUE_INDEX_HEAD = this.SOUND_QUEUE_INDEX_TAIL;
	}

	this.GET_SOUND_QUEUE_SAMPLES_ = function () {
		return  Math.abs(Math.abs(this.SOUND_QUEUE_SIZE - this.SOUND_QUEUE_INDEX_HEAD) - Math.abs(this.SOUND_QUEUE_SIZE - this.SOUND_QUEUE_INDEX_TAIL));
	}

	this.GET_SOUND_QUEUE_SAMPLES = function () {
		var RESULT = this.SOUND_QUEUE_INDEX_TAIL - this.SOUND_QUEUE_INDEX_HEAD;
		if (RESULT < 0) {
			RESULT = (RESULT + this.SOUND_QUEUE_SIZE) % this.SOUND_QUEUE_SIZE;
		}
		return RESULT;
	}

	this.PUT_SAMPLES_TO_SOUND_QUEUE = function (args_SAMPLES) {
		var COUNT = 0;
		for (var i=0; i<args_SAMPLES.length; i++) {
			if (this.SOUND_QUEUE_INDEX_HEAD == ((this.SOUND_QUEUE_INDEX_TAIL + 1) % this.SOUND_QUEUE_SIZE)) {
				console.log('SOUND_QUEUE_IS_FULL'); break;
			} else {
				this.SOUND_QUEUE[this.SOUND_QUEUE_INDEX_TAIL] = args_SAMPLES[i];
				this.SOUND_QUEUE_INDEX_TAIL = (this.SOUND_QUEUE_INDEX_TAIL + 1) % this.SOUND_QUEUE_SIZE;
				COUNT += 1;
			}
		}
		return COUNT;
	...
	this.GET_SAMPLES_FROM_SOUND_QUEUE = function (args_REQ_LENGTH) {
		var SAMPLES_FOR_RETURN = new Array();
		for (var i=0; i<args_REQ_LENGTH; i++) {
			if (this.SOUND_QUEUE_INDEX_HEAD == this.SOUND_QUEUE_INDEX_TAIL) {
				SAMPLES_FOR_RETURN.push(0.0);
	//			console.log('SOUND_QUEUE_IS_EMPTY');
			} else {
				SAMPLES_FOR_RETURN.push(this.SOUND_QUEUE[this.SOUND_QUEUE_INDEX_HEAD]);
				this.SOUND_QUEUE_INDEX_HEAD = (this.SOUND_QUEUE_INDEX_HEAD + 1) % this.SOUND_QUEUE_SIZE;
			}
		}
	//	console.log('GET_SAMPLES_FROM_SOUND_QUEUE : SIZE_IS : ' + SAMPLES_FOR_RETURN.length);
		return SAMPLES_FOR_RETURN;
	}

	this.PUT_UDP_JSON_SAMPLES_TO_SOUND_QUEUE = function (JSON_SAMPLES) {
		
		if (!('SAMPLE_BASE64' in JSON_SAMPLES)) return;

		var PCM_DATA = atob(JSON_SAMPLES.SAMPLE_BASE64);
		var SAMPLES_RECEIVE = this.BINARY_STRING_TO_Array(PCM_DATA);
		
		// 업샘플링 알고리즘 더욱 확실한거 없나? 아직 음질이 안좋아...
		var UP_SAMPLE_COUNT = 48000 / JSON_SAMPLES.SAMPLE_FREQ;
		var SAMPLES_48000 = new Array();
		for (var i=0; i<SAMPLES_RECEIVE.length; i++) {
			var DIFF_FROM_NEXT_VALUE = 0;
			if (i<SAMPLES_RECEIVE.length-1) {
				DIFF_FROM_NEXT_VALUE = SAMPLES_RECEIVE[i+1] - SAMPLES_RECEIVE[i];
			}
			for (var j=0; j<UP_SAMPLE_COUNT; j++) {
				SAMPLES_48000.push(
					(SAMPLES_RECEIVE[i] + (DIFF_FROM_NEXT_VALUE / UP_SAMPLE_COUNT) * j) / 0x8000
				);
			}
		}
		
		if (JSON_SAMPLES.SAMPLE_INDEX < 1) this.CLEAR_SOUND_QUEUE();
		var COUNT = this.PUT_SAMPLES_TO_SOUND_QUEUE(SAMPLES_48000);
		
	}

	this.NAUTES_WAE_AudioContext = new (window.AudioContext || window.webkitAudioContext)();

	this.analyser = this.NAUTES_WAE_AudioContext.createAnalyser();
	this.analyser.connect(this.NAUTES_WAE_AudioContext.destination);
	...
	this.SMOOTHING = 0.0;
	this.FFT_SIZE = 512;
	this.CANVAS = document.querySelector('.CANVAS_' + _THIS.DOM_ID_FOR_RENDERING);

	var bufferSize = 512;
	this.CONTEXT_SOUND_OUT = this.NAUTES_WAE_AudioContext.createScriptProcessor(bufferSize, 2, 2);
	this.CONTEXT_SOUND_OUT.onaudioprocess = function(audioProcessingEvent) {
		...
		var SAMPLES_FROM_QUEUE = _THIS.GET_SAMPLES_FROM_SOUND_QUEUE(inputBuffer.length);
		// Loop through the output channels (in this case there is only one)
		for (var channel = 0; channel < outputBuffer.numberOfChannels; channel++) {
			var inputData	= inputBuffer.getChannelData(channel);
			var outputData	= outputBuffer.getChannelData(channel);

			// Loop through the bufferSize samples
			for (var sample = 0; sample < inputBuffer.length; sample++) {
				// make output equal to the same as the input
				outputData[sample] = SAMPLES_FROM_QUEUE[sample];
//				outputData[sample] = (Math.sin((sample) * 8 * Math.PI / 180) * 2 - 1) * 0.01;
				// add noise to each output sample
//				outputData[sample] += ((Math.random() * 2) - 1) * 0.002;
			}
		}
	}

	this.DRAW_REALTIME_SOUND = function() {
		...
		for (var i = 0; i < this.analyser.frequencyBinCount; i++) {
			var offset = (this.times[i] - 128) + (this.CANVAS.height / 2);
			var barWidth = this.CANVAS.width/this.analyser.frequencyBinCount;
			drawContext.fillStyle = 'yellow';
			drawContext.fillRect(i * barWidth, offset, 1, 2);
		}

		NAUTES_SOUND_VISUALIZER_UPDATE_CANVAS(this.DRAW_REALTIME_SOUND.bind(this));
	}

	this.WAE_PLAYLIST = WaveformPlaylist.init({
		ac: _THIS.NAUTES_WAE_AudioContext,
		analyser: _THIS.analyser,
		samplesPerPixel: 2000,
		zoomLevels: [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000],
		waveHeight: 100,
		container: document.getElementById("NAUTES_WAE_PLAYLIST"),
		timescale: true,
//		mono: true,
		state: 'cursor',
		colors: {
		...
		controls: {
			show: TRACK_CONTROLLER_ENABLE,	//whether or not to include the track controls
			width: WIDTH_TRACK_CONTROLLER	//width of controls in pixels
		},
		seekStyle : 'line'
	});
	//initialize the WAV exporter.
	this.WAE_PLAYLIST.initExporter();
	this.WAE_EE = this.WAE_PLAYLIST.getEventEmitter();
//	this.TRACK_AUDIO_LENGTH	= -1;
	this.MAX_AUDIO_LENGTH	= 0;
	this.isLooping			= false;
	this.playoutPromises	= null;;
	this.SELECT_START		= 0;
	this.SELECT_END			= 0;

	this.WAE_EE.on('loadprogress', function (percent, src) {
		if (!WAVE_DISPLAY_ONLY) {
			$("#WAE_PROGRESSBAR_" + _THIS.DOM_ID_FOR_RENDERING).jqxProgressBar('val', percent);
			$("#WAE_AUDIO_POSTION_" + _THIS.DOM_ID_FOR_RENDERING).html(sprintf('%5.2f', percent) + ' %');
		}
	}).on('timeupdate', function (playbackPosition) {
		if (!WAVE_DISPLAY_ONLY) {
			$("#WAE_AUDIO_POSTION_" + _THIS.DOM_ID_FOR_RENDERING).html(cueFormatters('hh:mm:ss.uuu')(playbackPosition));
			$("#WAE_PROGRESSBAR_" + _THIS.DOM_ID_FOR_RENDERING).jqxProgressBar('val', playbackPosition / _THIS.MAX_AUDIO_LENGTH * 100);
//			console.log((playbackPosition / _THIS.MAX_AUDIO_LENGTH * 100));
			if ((playbackPosition / _THIS.MAX_AUDIO_LENGTH * 100) > 95.0) {
				$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-play">');
			}
		}
	}).on('scroll', function (scrollLeft) {
//		console.log('ON SCROLL ' + scrollLeft + ' : ' + _THIS.WAE_PLAYLIST.viewDuration + ' : ' + _THIS.WAE_PLAYLIST.duration);
	}).on("select", function(start, end) {
		_THIS.SELECT_START	=	start;
		_THIS.SELECT_END	=	end;
//		console.log(sprintf('START : %5.3f, END: %5.3f', start, end));
	}).on("mute", function(track) {
	  _THIS.displaySoundStatus("Mute button pressed for " + track.name);
	}).on("solo", function(track) {
	  _THIS.displaySoundStatus("Solo button pressed for " + track.name);
	}).on("volumechange", function(volume, track) {
	  _THIS.displaySoundStatus(track.name + " now has volume " + volume + ".");
	}).on("mastervolumechange", function(volume) {
	  _THIS.displaySoundStatus("Master volume now has volume " + volume + ".");
	}).on("audiorequeststatechange", function(state, src) {
		var name = src;
		if (src instanceof File) {
			name = src.name;
		}
		_THIS.displayLoadingData("Track " + name + " is in state " + audioStates[state]);
	}).on("audiosourcesloaded", function() {
		_THIS.displayLoadingData("Tracks have all finished decoding.");
	}).on("audiosourcesrendered", function() {
		_THIS.displayLoadingData("Tracks have been rendered");
		if (_THIS.MAX_AUDIO_LENGTH < _THIS.WAE_PLAYLIST.duration) _THIS.MAX_AUDIO_LENGTH = _THIS.WAE_PLAYLIST.duration;
		if (!WAVE_DISPLAY_ONLY) {
			$("#WAE_AUDIO_POSTION_" + _THIS.DOM_ID_FOR_RENDERING).html('00:00:00.000');
			$("#WAE_PROGRESSBAR_" + _THIS.DOM_ID_FOR_RENDERING).jqxProgressBar('val', 0);
		}
	}).on('audiorenderingfinished', function (type, data) {
		if (type == 'wav'){
			if (this.downloadUrl) {
			  window.URL.revokeObjectURL(this.downloadUrl);
			}
			this.downloadUrl = window.URL.createObjectURL(data);
			_THIS.displayDownloadLink(this.downloadUrl);
		}
		...
		_THIS.WAE_EE.emit("automaticscroll", false);
		if (_THIS.isLooping) {
//			console.log("_THIS.isLooping!");
			$('#BTN_NAUTES_WAE_EXEC_PLAY_TOGGLE').html('<i class="fa fa-pause">');
			if (_THIS.playoutPromises != null) {
				_THIS.WAE_EE.emit("automaticscroll", true);
				_THIS.playoutPromises.then(function() {
					  _THIS.playoutPromises = _THIS.WAE_PLAYLIST.play(_THIS.SELECT_START, _THIS.SELECT_END);
				});
			}
		}
	...
	this.displaySoundStatus = function (status) {
		console.log(status);
		$(".sound-status").html(status);
	}

	this.displayLoadingData = function (data) {
		console.log(data);
		var info = $("<div/>").append(data);
		$(".loading-data").append(info);
	}

	this.displayDownloadLink = function (link) {
		if (SAVE_AS.length <= 0) {
			SAVE_AS = 'MIX_WAV_' + (new Date()).toISOString() + '.wav';
		}
		var dateString = (new Date()).toISOString();
		var $link = $("<a/>", {
			'href': link,
			'download': SAVE_AS,
			'text': 'Download ' + SAVE_AS,
	...
	this.CONTEXT_SOUND_OUT.connect(this.analyser);
	NAUTES_SOUND_VISUALIZER_UPDATE_CANVAS(this.DRAW_REALTIME_SOUND.bind(this));

//	console.log(JSON.stringify(this.WAE_EE, null, '\t'));
...
//	  console.log(JSON.stringify(config, null, '\t'));
	  ...
	  if (config.analyser != undefined) playlist.analyser = config.analyser;
	  playlist.playSpeed = 1.0;
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    value: function setUpEventEmitter() {
	      ...
	      ee.on('track-delete', function () {
	        var track = _this2.getActiveTrack();
		    for(var i=0; i<_this2.tracks.length; i++) {
		    	if (track === _this2.tracks[i]) {
		    		_this2.tracks.splice(i,1);
			        _this2.drawRequest();
		    		break;
		    	}
		    }
	      ...
	      ee.on('trim', function () {
	        ...
	        track.trim(timeSelection.start, timeSelection.end, _this2);
	    ...
	    value: function load(trackList) {
	      ...
	      var loadPromises = trackList.map(function (trackInfo) {
	        var loader = _LoaderFactory2.default.createLoader(trackInfo.src, _this3.ac, _this3.ee, trackInfo.CUES);
	        return loader.load(trackInfo.CUES);
	      });
	
	      return Promise.all(loadPromises).then(function (audioBuffers_WITH_CUES) {
	        _this3.ee.emit('audiosourcesloaded');
	        
	        var tracks = audioBuffers_WITH_CUES.map(function (audioBuffer_WITH_CUES, index) {
	          ...
	          var cueOut = info.cueout || audioBuffer_WITH_CUES.audioBuffer.duration;

		      if (audioBuffer_WITH_CUES.CUES != undefined) {
//console.log(sprintf('track.setCues() : %s', JSON.stringify(audioBuffer_WITH_CUES.CUES, null, '\t')));
				cueIn	= audioBuffer_WITH_CUES.CUES.start;
				cueOut	= audioBuffer_WITH_CUES.CUES.end;
//		      	track.setCues(audioBuffer_WITH_CUES.CUES.start, audioBuffer_WITH_CUES.CUES.end);
		      }
	          ...
	          var playout = new _Playout2.default(_this3.ac, audioBuffer_WITH_CUES.audioBuffer);
	
	          var track = new _Track2.default();
	          track.src = info.src;
	          track.setBuffer(audioBuffer_WITH_CUES.audioBuffer);
	          track.setName(name);
	    ...
	    */
	    ...
	    value: function isSegmentSelection() {
	      return this.timeSelection.start !== this.timeSelection.end; // THIS OLNY LINE IS ORIGINAL
	    }
	    ...
	    */
	    ...
	    value: function setTimeSelection() {
	      ...
	      var FLAG_MULTI_SELECTION = false;
	      if ((track != undefined) && (track.BUTTON_active != undefined)) {
	      	if (track.BUTTON_active == 'RIGHT') FLAG_MULTI_SELECTION = true;
// console.log(sprintf('timeSelection : %s %s', track.BUTTON_active, JSON.stringify(this.timeSelection, null, '\t')));
	      }
	      
	      if (FLAG_MULTI_SELECTION) {
	      	track.MULTI_timeSelection[track.MULTI_timeSelection_INDEX] = {
		        start: start,
		    ...
		    this.timeSelection = {
			  ...
			  this.MULTI_timeSelection = undefined;
		      this.timeSelection = {
		        start: start,
		        end: end === undefined ? start : end
		      };
	    ...
	    value: function startOfflineRender(type) {
	      ...
	      if (this.duration <= 0) {
	      	console.log('AUDIO DURATION IS LEESS THEN OR EQUAL TO 0!');
	      	return;
	      }
	    ...
	    */
	    ...
	    value: function seek(start, end, track) {
	      if (this.isPlaying()) {
	        ...
	        this.pausedAt = start;
	        this.setTimeSelection(start, end, track);
	    ...
	    */
	    ...
	    value: function updateEditor(cursor) {
	      ...
	      if (this.isPlaying()) {
			...
			this.tracks.forEach(function (track) {
				if (track.MULTI_timeSelection != undefined) {
					for(var i=0; i<track.MULTI_timeSelection.length; i++) {

						if ((_this14.playbackSeconds > track.MULTI_timeSelection[i].end) && i < (track.MULTI_timeSelection.length - 1)) continue;
//console.log(sprintf('updateEditor : (%5.2f, %5.2f, %5.2f);', _this14.playbackSeconds, track.MULTI_timeSelection[i].start, track.MULTI_timeSelection[i].end));
						if ((_this14.playbackSeconds > track.MULTI_timeSelection[i].start) && (_this14.playbackSeconds <= track.MULTI_timeSelection[i].end)) {
							var INDEX_OF_MUTED_TRACK = _this14.mutedTracks.indexOf(track);
							if (INDEX_OF_MUTED_TRACK >= 0) { 
								// UN_MUTE
							    _this14.muteTrack(track);
							    _this14.adjustTrackPlayout();
							    _this14.drawRequest();
							}
						} else {
							var INDEX_OF_MUTED_TRACK = _this14.mutedTracks.indexOf(track);
	    ...
	    value: function isActiveTrack(track) {
		  ...
		  if (track.MULTI_timeSelection == null) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function () {
	  ...
	  _createClass(_class, null, [{
	    key: 'createLoader',
	    value: function createLoader(src, audioContext, ee, CUES) {
	      if (src instanceof Blob) {
	        return new _BlobLoader2.default(src, audioContext, ee, CUES);
	      } else if (typeof src === 'string') {
	        return new _XHRLoader2.default(src, audioContext, ee, CUES);
	      }
...
//console.log(sprintf('Promise(CUES) => : %s', JSON.stringify(CUES, null, '\t')));
	              	resolve({ audioBuffer: audioBuffer, CUES: CUES });
	            });
	          });
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    */
	    ...
	    key: 'trim',
	    value: function trim(start, end, _THIS) {
	    	if (this.MULTI_timeSelection != undefined) {
	    	  for(var i=0; i<this.MULTI_timeSelection.length; i++) {
	    	  	if (i < 1) {
	    	  		this.setCues(this.MULTI_timeSelection[i].start, this.MULTI_timeSelection[i].end);
	    	  	} else {
					_THIS.load([{
						src: this.src,
						name: this.src.name,
						CUES: this.MULTI_timeSelection[i]
					}]);
	    	  	}
	    	  }
	    	  this.MULTI_timeSelection = undefined;
	    ...
	    */
	    ...
	    value: function schedulePlay(now, startTime, endTime, config) {
	      ...
	      var sourcePromise = playoutSystem.setUpSource(config._THIS, config.isOffline);
...
console.log(sprintf('playoutSystem.play(%5.2f, %5.2f, %5.2f);', when, start, duration));
		  if (config.track.MULTI_timeSelection != undefined) {
//			config.track.MULTI_timeSelection = ADJUST_MULTI_SELECTION(config.track.MULTI_timeSelection);
//console.log(sprintf('AFTER ADJUST_MULTI_SELECTION %s', JSON.stringify(config.track.MULTI_timeSelection, null, '\t')));
		  }
	    ...
	    value: function render(data) {
	      ...
	      if (data.isActive === true) {
      		if (data.track.MULTI_timeSelection != null) {
	      		data.track.MULTI_timeSelection.forEach(function (ONE_timeSelection) {
			        var cStartX = (0, _conversions.secondsToPixels)(ONE_timeSelection.start, data.resolution, data.sampleRate);
			        var cEndX = (0, _conversions.secondsToPixels)(ONE_timeSelection.end, data.resolution, data.sampleRate);
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    value: function mousedown(e) {
	      e.preventDefault();
	      if (e.which == 1) {
	      	this.track.BUTTON_active = 'LEFT';
	      } else if (e.which == 2) {
	      } else if (e.which == 3) {
	      	this.track.BUTTON_active = 'RIGHT';
	      }

		  if (this.track.BUTTON_active == 'RIGHT') {
			if (this.track.MULTI_timeSelection == undefined) {
				this.track.MULTI_timeSelection = [];
				this.track.MULTI_timeSelection_INDEX = 0;
				this.track.MULTI_timeSelection.push({
				  start: 0,
				  end: 0
				});
			}
		  } else {
			this.track.MULTI_timeSelection = undefined;
			this.track.MULTI_timeSelection_INDEX = undefined;
		  }
		  this.active = true;
	    ...
	    value: function mouseup(e) {
	      if (this.active) {
	        e.preventDefault();
			if (this.track.BUTTON_active == 'RIGHT') {
	      	  this.track.MULTI_timeSelection_INDEX += 1;
		      this.track.MULTI_timeSelection.push({
		        start: 0,
		        end: 0
			  });
// console.log(sprintf('this.track.MULTI_timeSelection %s', JSON.stringify(this.track.MULTI_timeSelection, null, '\t')));
			  this.complete(e.offsetX);
			  this.track.ee.emit('select', 0, 0, this.track);
			  this.track.MULTI_timeSelection = ADJUST_MULTI_SELECTION(this.track.MULTI_timeSelection);
			} else {
			  this.complete(e.offsetX);
			}
	      }
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    key: 'setUpSource',
	    value: function setUpSource(args_THIS, isOffline) {
	      var _this = this;
	
	      this.source = this.ac.createBufferSource();
	      this.source.buffer = this.buffer;

    	  if (args_THIS.playSpeed != undefined) this.source.playbackRate.value = args_THIS.playSpeed;
	      ...
	      if (isOffline) {
			  ...
			  if (args_THIS.analyser != undefined) {
			      this.masterGain.connect(args_THIS.analyser);
			  } else {
			      this.masterGain.connect(this.destination);
			  }
	      }
