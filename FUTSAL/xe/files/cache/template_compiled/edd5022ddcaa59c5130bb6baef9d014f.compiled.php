	...
	QIIP_API_ACCESS({
			...
			if ('errors' in JSON_RESULT){
				...
				if (ERROR_REASON == 'invalidcombo') {
					STR_ERROR_MESSAGE = '입력하신 내용이 올바르지 않습니다.';
				}
...
<div class="login_table">
	<header>
		<div class="container">
			<button class="BTN_PW_RESET_FORM" id="BTN_TO_BACK"><i class="fa fa-arrow-left" aria-hidden="true"></i> 뒤로</button>
		</div>
	</header>
	<div class="container join_form">
		<h2>{{PHONE_INFO.APP_NAME}} 비밀번호 재설정</h2>
        <div class="form-group join_in">
    		<input type="email" class="form-control" placeholder="핸드폰 번호 또는 이메일 주소" id="USER_EMAIL">
            <button type="submit" class="btn btn-primary block full-width m-b BTN_PW_RESET_FORM" id="BTN_TO_PW_RESET">비밀번호 재설정 메일 발송 요청</button>
        </div>
    </div>
</div>
