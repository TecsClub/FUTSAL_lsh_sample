...
function LAUNCH_CALL_OUT_POPUP_WINDOW() {
	...
	QIIP_API_ACCESS({
			...
			FILE_TEMPLATE: 'PAGES/APP_CONTENTS_CALL_OUT.html',
...
function IS_VALID_REMOTE_FROM(JSON_ARGS) {
	var RESULT = false;
	_RS.APP_CONFIG.IVR_SERVER_IPS.forEach(function (ONE_IVR_SERVER) {
...
function UPDATE_CALL_GRID_STATUS(JSON_PARSED) {
	var GRID_BOX_LAST;
	if (JSON_PARSED.CALL_IN_BOUND) {
		GRID_BOX_LAST = parseInt(JSON_PARSED.CALL_ARGS.CALL_FROM.slice(-1));
	} else {
		GRID_BOX_LAST = parseInt(JSON_PARSED.CALL_ARGS.CALL_TO.slice(-1));
	}
	var INDEX_FOUNDED = -1;
			...
			if (CALL_GRID_STATUS[i].CALL_ARGS.UNIQUE_ID == JSON_PARSED.CALL_ARGS.UNIQUE_ID) {
				if (FLAG_CALL_AMOUNT_WINDOW) {
					if (!JSON_PARSED.CALL_END) {
						if (!CALL_GRID_STATUS[i].CALL_CONNECT && JSON_PARSED.CALL_CONNECT) {
							UPDATE_CALL_AMOUNT_CHART_CHANGE_2_CONNECT(JSON_PARSED);
						}
					} else {
						if (JSON_PARSED.CALL_IN_BOUND) {
							UPDATE_CALL_AMOUNT_CHART_REMOVE_CONNECT(JSON_PARSED);
						} else {
							if ('SCENARIO_HISTORY' in JSON_PARSED) {
								if (JSON_PARSED.SCENARIO_HISTORY.length > 0) {
									UPDATE_CALL_AMOUNT_CHART_REMOVE_CONNECT(JSON_PARSED);
								} else {
									UPDATE_CALL_AMOUNT_CHART_REMOVE_TASK(JSON_PARSED);
								}
							}
/*							
							if (CALL_GRID_STATUS[i].CALL_CONNECT) {
								UPDATE_CALL_AMOUNT_CHART_REMOVE_CONNECT(JSON_PARSED);
							} else {
								UPDATE_CALL_AMOUNT_CHART_REMOVE_TASK(JSON_PARSED);
							}
*/
	...
	if (INDEX_FOUNDED < 0) {
		...
		if (FLAG_CALL_AMOUNT_WINDOW) {
			UPDATE_CALL_AMOUNT_CHART_NEW_TASK_OR_CONNECT(JSON_PARSED);
		}
	}
...
function GET_HTML_STATUS(JSON_PARSED) {
	var STR_HTML = '';
		...
		STR_HTML += '	<tr>';
		STR_HTML += '		<td style="text-align:center; background:white; color:black; font-weight:normal; font-size:0.8em;">';
		STR_HTML += '			<button class="BTN_IVR_MONITOR" id="BTN_CALL_PLAYBACK" ';
		STR_HTML += '				IVR_ADDR="' + JSON_PARSED.REMOTE_ADDR + '" ';
		STR_HTML += '				CALL_END="' + JSON_PARSED.CALL_END + '" ';
		STR_HTML += '				CALL_FILE_PATH_PREFIX="' + sprintf('%s/%s', JSON_PARSED.CALL_STATUS.REC_FOLDER, JSON_PARSED.CALL_STATUS.REC_FILE_PREFIX) + '" >';
		STR_HTML += '				청취하기';
		STR_HTML += '			</button>';
		STR_HTML += '		</td>';
		STR_HTML += '	</tr>';
