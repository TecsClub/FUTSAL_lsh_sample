...
function SET_DOM_EVENT() {
	$('.ITS_WORK_DAY').on('click', function () {
		...
		if (ID == 'DUMMY') {
			...
			var STR_TEXT = $(this).is(':checked') ? '특근' : '임시휴무';
...
function EXEC_ADD_GROUP() {
	...
	if (FLAG_EXIST) {
		QIIP_APP_ALERT([
			sprintf('%s 는 이미 존재하는 그룹명 입니다.', TXT_ADD_GROUP)
		]);
		return;
	}
	$('#TXT_ADD_GROUP').val('');
...
function EXEC_ADD_NUMBER() {
	...
	if (FLAG_EXIST) {
		QIIP_APP_ALERT([
			sprintf('%s 는 이미 존재하는 전화번호 입니다.', TXT_ADD_NUMBER)
		]);
		return;
	}
	$('#TXT_ADD_NUMBER').val('');
...
function EXEC_ADD_BANK() {
	...
	if (FLAG_EXIST) {
		QIIP_APP_ALERT([
			sprintf('%s 는 이미 존재하는 은행명 입니다.', TXT_ADD_BANK)
		]);
		return;
	}
	$('#TXT_ADD_BANK').val('');
...
function EXEC_ADD_CARD() {
	...
	if (FLAG_EXIST) {
		QIIP_APP_ALERT([
			sprintf('%s 는 이미 존재하는 카드사명 입니다.', TXT_ADD_CARD)
		]);
		return;
	}
	$('#TXT_ADD_CARD').val('');
...
<div class="ars_config">
	<div style="height:100%; width:100%;">
		<div style="width:100%;height:10%;">
			<input class="BTN_ARS_CONFIG" style="width:100%;height:100%;" type="button" value="설정사항 저장" id="BTN_SAVE_CUSTOM_WORKS"/>
		</div>
					...
					<div style="width:100%;height:40%;">
						<table style="width:100%;height:100%;" class="ORDINARY_WORK_INFORMATION">
							...
							<tr class="ORDINARY_WORK_TIMES" ng-repeat="ONE_WORK_TIME in ARS_CONFIG.WORK_INFORMATION.WORK_TIME_INFORMATION.WORK_TIMES">
								<th>업무시간 - {<?php echo $__Context->index + 1 ?>}</th>
								<td>
						...
						<div style="width:50%;height:100%;float:left;">
							<div style="width:100%;height:25%;">
								<table style="height:100%;">
									...
									<tr>
										<td colspan="3">
											<input type="text" style="width:100%;height:100%;text-align:center;" id="TXT_ADD_GROUP" placeholder="추가할 그룹명 입력"/>
										</td>
									</tr>
									<tr>
										<td>
										...
										<td>
											<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_ADD_GROUP" value="추가"/>
										</td>
										<td>
											<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_DEL_GROUP" value="삭제"/>
										</td>
									</tr>
								</table>
							</div>
						...
						<div style="width:50%;height:100%;float:left;">
							<div style="width:100%;height:25%;">
								<table style="height:100%;">
									<tr>
										<td colspan="2">
											<input type="number" style="width:100%;height:100%;text-align:center;" id="TXT_ADD_NUMBER" placeholder="추가할 전화번호 입력"/>
										</td>
									</tr>
									<tr>
										<td>
											<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_ADD_NUMBER"  value="추가"/>
										</td>
										<td>
											<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_DEL_NUMBER" value="삭제"/>
										</td>
									</tr>
								</table>
							</div>
							...
							<tr class="CUSTOM_WORKS" >
								<td>
									<table>
										...
										<tr>
											<th class="LABEL_ITS_WORK_DAY_ADD">특근</th>
											<td>
							...
							<tr class="CUSTOM_WORKS" ng-repeat="ONE_WORK in ARS_CONFIG.WORK_INFORMATION.CUSTOM_WORK_INFORMATION">
								<td>
									<table>
										...
										<tr>
											<th>날짜 - {<?php echo $__Context->index + 1 ?>}</th>
											<td><div class="CUSTOM_WORK_DATE" value="{{ONE_WORK.DATE}}"></div></td>
										</tr>
										<tr>
											<th class="LABEL_ITS_WORK_DAY_{<?php echo $__Context->index + 1 ?>}" ng-show="!ONE_WORK.ITS_WORK_DAY">임시휴무</th>
											<th class="LABEL_ITS_WORK_DAY_{<?php echo $__Context->index + 1 ?>}" ng-show="ONE_WORK.ITS_WORK_DAY">특근</th>
											<td>
					...
					<div style="width:100%;height:90%;">
						<div style="width:100%;height:25%;">
							<table style="height:100%;">
								<tr>
									<td colspan="2">
										<input type="text" style="width:100%;height:100%;text-align:center;" id="TXT_ADD_BANK" placeholder="추가할 은행명 입력"/>
									</td>
								</tr>
								<tr>
									<td>
										<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_ADD_BANK" value="추가"/>
									</td>
									<td>
										<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_DEL_BANK" value="삭제"/>
									</td>
								</tr>
							</table>
						</div>
					...
					<div style="width:100%;height:90%;">
						<div style="width:100%;height:25%;">
							<table style="height:100%;">
								<tr>
									<td colspan="2">
										<input type="text" style="width:100%;height:100%;text-align:center;" id="TXT_ADD_CARD" placeholder="추가할 카드사 입력"/>
									</td>
								</tr>
								<tr>
									<td>
										<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_ADD_CARD" value="추가"/>
									</td>
									<td>
										<input class="BTN_ARS_CONFIG" type="button" style="width:100%;height:100%;" id="BTN_DEL_CARD" value="삭제"/>
									</td>
								</tr>
							</table>
						</div>
